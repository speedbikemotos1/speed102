...
import { Plus, Edit2, Trash2, Split, DollarSign, TrendingUp } from "lucide-react";
import Dashboard from "./Dashboard";
import { useToast } from "@/hooks/use-toast";
import { useCreateDeferredSale, useDeferredSales, useDeleteDeferredSale, useUpdateDeferredSale } from "@/hooks/use-deferred";
...
export default function DifferPage() {
  ...
  const totalAmount = useMemo(() => rows.reduce((acc, r) => acc + Number(r.montant), 0), [rows]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsOpen(false);
      setEditingId(null);
      setFormData({ date: new Date().toISOString().split('T')[0], nomPrenom: "", numeroTelephone: "", typeMoto: "", designation: "", montant: 0, isSettled: false });
      toast({ title: "Succès" });
    } catch (err: any) {
    ...
    <Dashboard contentOnly={true}>
      <div className="flex flex-col gap-8 p-8 max-w-7xl mx-auto bg-gradient-to-br from-gray-50 via-white to-gray-100 min-h-screen animate-enter">
        ...
        <div className="grid gap-6 md:grid-cols-4">
          ...
          <Card className="bg-white/70 backdrop-blur-sm border-gray-200 shadow-sm hover:shadow-xl transition-all rounded-3xl group">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-xs font-black text-gray-500 uppercase tracking-widest">Ventes</CardTitle>
          ...
          <Card className="bg-white/70 backdrop-blur-sm border-gray-200 shadow-sm hover:shadow-xl transition-all rounded-3xl group">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-xs font-black text-gray-500 uppercase tracking-widest">Montant</CardTitle>
